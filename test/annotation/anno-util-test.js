...
describe('mergeTargets', function() {
  it('merge single target into single, same canvas', function() {
    const anno = {
      on: {
        full: '/canvas1',
        selector: {
    ...
    expect(anno.on.selector.value).to.equal('<svg><g><path d="M1,1,2,1,2,2z"/><path d="M4,1,5,1,5,2z"/></g></svg>');
  });
  
  it('merge single target into single, different canvases', function() {
    const anno = {
      on: {
        full: '/canvas1',
        selector: {
    ...
    annoUtil.mergeTargets(anno, sourceTarget);
    expect(anno.on).to.be.instanceof(Array);
  ...
  it ('merge single target into multiple, same canvas', function() {
    const anno = {
      on: [
        {
          full: '/canvas1',
    ...
    expect(anno.on[0].selector.value).to.equal('<svg><path d="M1,1,2,1,2,2z"/></svg>');
    expect(anno.on[1].full).to.equal('/canvas2');
    expect(anno.on[1].selector.value).to.equal('<svg><g><path d="M5,1,6,1,6,2z"/><path d="M4,1,5,1,5,2z"/></g></svg>');
  });
  
  it ('merge single target into multiple, different canvas', function() {
    const anno = {
      on: [
        {
          full: '/canvas1',
